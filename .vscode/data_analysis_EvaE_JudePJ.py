...
class Date:  # Contains date data for easier accessibility
    def __init__(self, date_string):  # Creates date object based on a date string
        l = date_string.split(
            "-"
        )  # Splits something like "2024-02-08" to ["2024", "02", "08"], which are each parsed into ints
        self.day = int(l[2])
        self.month = int(l[1])
        self.year = int(l[0])
...
class Dog:  # Contains dog data
    def __init__(
        self, info
    ):  # Takes important info from each row. We can modify later if we want to get different data from the file
        self.code = info[1]
        self.name = info[2]
        self.breed = info[3]
        self.colours = info[4]
...
def max_min_date(when, string):
    print(
        f"The maximum {string} where an attack occurred is {max(when)}, and the minimum is {min(when)}"
    )


def latest_date(year, month, day):  # Formats the date
    print(
        f"The most recent day where a dog attack occurred was on {month_num_to_str(month)} {day}, {year}"
    )


def first_date(
    year, month, day
):  # Formatting. We could probably combine the two functions but this seems a bit clearer for this purpose
    print(
        f"The first day where a dog attack occurred was on {month_num_to_str(month)} {day}, {year}"
    )


def format_occurrence_tuple(
    tup,
):  # Formats a tuple in the form of (word, number of occurrences)
...
def split_values(
    value_list, s_char
):  # Splits a value like BROWN / TAN into ["BROWN", "TAN"] or PRIVATE PROPERTY to ["PRIVATE", "PROPERTY"], depending on s_char
    split_values = []
...
def main():
    ...
    with open("Dogs Issued Dangerous Dog Orders.csv", newline="") as file:  # Reads csv
        ...
        for (
            ...
            read
        ):  # Iterates over each row in file and creates a dog object for each and appends to dog list
            dog = Dog(row)
            dogs.append(dog)
    """
    DATA ANALYSIS
    ...
    month_counter = Counter(
        [(dog.date.year, dog.date.month) for dog in dogs]
    ).most_common()
    ...
    top_ten_words = (
        [dog.code for dog in dogs]
        + [dog.name for dog in dogs]
        + split_breeds
        + split_colours
        + [dog.ward_name for dog in dogs]
        + split_bites
        + split_locs
    )
    ...
    max_day = max(
        [
            dog.date.day
            for dog in dogs
            if dog.date.year == max_year and dog.date.month == max_month
        ]
    )
    ...
    min_day = min(
        [
            dog.date.day
            for dog in dogs
            if dog.date.year == min_year and dog.date.month == min_month
        ]
    )
    ...
    print(
        f"The highest monthly average amount of dog attacks is in {month_num_to_str(month_counts.index(max(month_counts)))}, with an average of {round(max(month_counts)/12, 2)} attacks per year"
    )

    # Lowest average attacks per month
    print(
        f"The lowest monthly average amount of dog attacks is in {month_num_to_str(month_counts.index(min(month_counts)))}, with an average of {round(min(month_counts)/12, 2)} attacks per year"
    )

    # Highest month of attacks ever
    print(
        f"The highest amount of dog attacks in a single month is {month_threshold}, which occurred in {month_num_to_str(highest_months[0][1])} {highest_months[0][0]}"
    )  # Did not want to both making this scalable because they aren't tied anyway

    # Highest year of attacks
    print(
        f"The highest amount of dog attacks in a single year is {year_threshold}, which was in {h_years_str}"
    )

    # Most common occurrences
    print(
        f"The top three most common names for dogs with the Dangerous Dog classification are {format_top_three(most_common_names)}"
    )
    print(
        f"The top three most common colours for dogs with the Dangerous Dog classification are {format_top_three(most_common_colours)}"
    )
    print(
        f"The top three most common breeds for dogs with the Dangerous Dog classification are {format_top_three(most_common_breeds)}"
    )
    ...
    print(
        f"There are {number_words} total words in all dog's colour values, and the average amount of words per dog colour value is {round(average_of_words, 2)}"
    )

    # Overall most common words
    print(
        f"The top ten most common words and the amount they're repeated in this file are: {top_words_formatted}"
    )
